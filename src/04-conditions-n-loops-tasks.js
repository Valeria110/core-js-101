...
function findFirstSingleChar(str) {
  return str.split('').find((item, _, arr) => arr.indexOf(item) === arr.lastIndexOf(item));
}
...
function getDigitalRoot(num) {
  const sum = Array.from(String(num)).reduce((acc, item) => acc + Number(item), 0);
...
function getCommonDirectoryPath(pathes) {
  ...
  const commonDir = arr[0].filter((el, ind) => arr.every((elem) => elem[ind] === el))
    .join('/');

  return commonDir + slash;
}
